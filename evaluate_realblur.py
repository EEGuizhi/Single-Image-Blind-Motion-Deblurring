...
"""
Filename: evaluate_realblur.py
Author: BSChen, JRKang
Description:
    NYCU IEE Deep Learning Final Project - Single Image Blind Motion Deblurring
    Group 25: 313510156, 313510217

    This script is modified based on the official implementation of Restormer.
"""
...
def compute_ssim(tar_img, prd_img, cr1):
    ssim_pre, ssim_map = structural_similarity(
        ...
        use_sample_covariance=False, data_range = 1.0, full=True, channel_axis=2
    )
...
if __name__ == "__main__":
    ...
    dataset = 'RealBlur_J'
    data_list_file = TEST_J_FILE.replace('.txt', '.json')
    ...
    for idx, data in tqdm(enumerate(json_list), desc="Reading image info"):
        ...
        pred_image_path = gt_image_path.replace('/gt/', '/pred/').replace('gt_', 'sharp_')

        gt_list.append(gt_image_path)
        path_list.append(pred_image_path)
