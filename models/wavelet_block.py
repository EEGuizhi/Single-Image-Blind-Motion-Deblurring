...
class DWT(nn.Module):
    ...
    def forward(self, x):
        ...
        for _ in range(self.level):
            ...
            res = h_component.view(b, c, 4, h // 2, w // 2)
...
class IDWT(nn.Module):
    ...
    def forward(self, x, weight=None):
        ...
        for c_pos, component_lh_hl_hh in enumerate(x[1:]):
            ...
            l_component = l_component.view(l_component.size(0), -1, l_component.size(3), l_component.size(4))
...
class FFT3(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_patch = x.view(x.size(0), x.size(1), H // 8, 8, W // 8, 8).permute(0, 1, 2, 4, 3, 5).contiguous()
        ...
        x = x_patch.permute(0, 1, 2, 4, 3, 5).contiguous().view(x.size(0), x.size(1), H, W)
        x = self.conv2(x)
        x = self.act(x)
        x = self.conv3(x)
        return x
