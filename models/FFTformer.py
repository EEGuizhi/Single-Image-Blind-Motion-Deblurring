...
Description:
    ...
    https://github.com/kkkls/FFTformer/blob/main/basicsr/models/archs/fftformer_arch.py
"""
...
def to_3d(x: torch.Tensor):
    # return rearrange(x, 'b c h w -> b (h w) c')
    return x.permute(0, 2, 3, 1).reshape(x.size(0), -1, x.size(1))


def to_4d(x: torch.Tensor, h: int, w: int):
    # return rearrange(x, 'b (h w) c -> b c h w', h=h, w=w)
    return x.reshape(x.size(0), h, w, x.size(2)).permute(0, 3, 1, 2)
...
class FSAS(nn.Module):
    ...
    def forward(self, x):
        ...
        # )
        b, c, h, w = q.shape
        q_patch = q.view(b, c,                        # (B, C, h, ps, w, ps)
            h // self.patch_size, self.patch_size,
            w // self.patch_size, self.patch_size
        )
        q_patch = q_patch.permute(0, 1, 2, 4, 3, 5)   # (B, C, h, w, ps, ps)
        q_patch = q_patch.contiguous()

        # k_patch = rearrange(
        #     k, 'b c (h patch1) (w patch2) -> b c h w patch1 patch2',
        #     patch1=self.patch_size, patch2=self.patch_size
        # )
        k_patch = k.view(b, c,                        # (B, C, h, ps, w, ps)
            h // self.patch_size, self.patch_size,
            w // self.patch_size, self.patch_size
        )
        k_patch = k_patch.permute(0, 1, 2, 4, 3, 5)   # (B, C, h, w, ps, ps)
        k_patch = k_patch.contiguous()
        ...
        #     patch1=self.patch_size, patch2=self.patch_size
        # )
        out = out.permute(0, 1, 2, 4, 3, 5).contiguous()   # (B, C, h, ps, w, ps)
