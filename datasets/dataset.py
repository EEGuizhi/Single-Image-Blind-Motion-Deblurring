...
def padding(
    img_lq: np.ndarray, img_gt: np.ndarray, tgt_size: tuple[int, int]
) -> tuple[np.ndarray, np.ndarray]:
    h, w, _ = img_lq.shape

    h_pad = max(0, tgt_size[0] - h)
    w_pad = max(0, tgt_size[1] - w)
...
class RealBlurDataset(Dataset):
    ...
    def __init__(
        ...
        for idx, data in tqdm(enumerate(json_list), desc="Reading image info"):
            ...
            if self.orig_size or self.rand_crop:
                # One patch only
                ...
                h_indices = list(range(0, max(1, h - self.img_size[0] + 1), h_step))
                w_indices = list(range(0, max(1, w - self.img_size[1] + 1), w_step))
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, int]:
        ...
        if not self.orig_size and (h < self.img_size[0] or w < self.img_size[1]):
            # Pad image if smaller than target size
            blur_img, gt_img = padding(blur_img, gt_img, self.img_size)

        if self.orig_size or gt_img.shape[:2] == self.img_size:
        ...
        if self.with_path:
            return blur_img, gt_img, (h_idx, w_idx), img_idx, n_patches, self.blur_image_list[index][0]
        else:
...
class GoProDataset(Dataset):
    """GoPro Dataset for Blind Motion Image Deblurring Task
    Args:
    ...
    def __init__(
        ...
        print(f"[Dataset] Initializing GoPRODataset with split='{split}'")
        assert split in ["train", "test"], "Split must be 'train' or 'test'"
        
        data_list_file = f"{root}/GoPro_{split}_list.json"
        assert os.path.exists(data_list_file), f"Data list file {data_list_file} does not exist."
        ...
        self.gt_image_list   = []
        json_list = read_json_list(data_list_file)
        print(f"Total {len(json_list)} image pairs found.")
        ...
        # Finish initialization
        print(f"[Dataset] GoProDataset initialized with {len(self.blur_image_list)} samples.\n")
...
def custom_collate_fn(batch: list) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, int]:
    ...
    path      = [item[5] for item in batch] if len(batch[0]) > 5 else None
    if path is not None:
        return blur_imgs, gt_imgs, start_pos, img_idxs, n_patches, path
    else:
